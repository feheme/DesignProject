...
var mobile = (function () {
    ...
    var init$3 = function () {
      ...
      var menus = Cell({});
      var paths = Cell({});
      var info = Cell(Option.none());
      var directory = Cell({});
      var clear = function () {
        expansions.set({});
        menus.set({});
        paths.set({});
        info.set(Option.none());
      };
      var isClear = function () {
        return info.get().isNone();
      };
      var setContents = function (sPrimary, sMenus, sExpansions, dir) {
        info.set(Option.some(sPrimary));
      ...
      var getPrimary = function () {
        return info.get().bind(lookupMenu);
      };
    ...
    var make$3 = function (detail, rawUiSpec) {
      ...
      var setup = function (container) {
        ...
        layeredState.setContents(detail.data().info(), componentMap, detail.data().expansions(), directory);
      ...
      var events = derive([
        ...
        runOnAttached(function (container, simulatedEvent) {
          setup(container).each(function (info) {
            Replacing.append(container, premade$1(info));
            if (detail.openImmediately()) {
              setActiveMenu(container, info);
              detail.onOpenMenu()(container, info);
            }
          });
        })
        ...
        behaviours: deepMerge(derive$2([
          Keying.config({
            ...
            focusIn: function (container, keyInfo) {
              layeredState.getPrimary().each(function (info) {
                dispatch(container, info.element(), focusItem());
              });
            }
          }),
          Highlighting.config({
    ...
    var tieredData = function (info, menus, expansions) {
      return {
        info: info,
        menus: menus,
